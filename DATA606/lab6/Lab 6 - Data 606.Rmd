...
<div class="alert alert-success">
We must assume that the findings were: random sample from the entire population, the groups are independent of each other, the sample size should be large enough.
</div> 
...
<div class="alert alert-success">
In Table 6, each row corresponds to a country. 
In the atheism data set, each row corresponds to a respondent/individual person with responses only related to atheist and non-atheist.
</div> 
...
pus12athe$percent <- round(pus12athe$total / sum(pus12athe$total),2)
kable(pus12athe)
```

<div class="alert alert-success">
The percentage of 0.05 here does match the .05 estimate from table 6.
</div> 
...
* Success-failure condition: n*p = 1002 * 0.05 = 50.1 and n*(1-p) = 1002 * 0.95 = 951.9 => which are both > 10, therefore the condition is met.
</div> 
...
Given: $SE=0.0069$<br>
The margin of error is: $ME=`r 1.96*0.0069`$
</div> 
...
<div class="alert alert-success">
95% confidence => $z=1.96$<br>
Given: $SE_{China}=0.0223$, $SE_{India}=0.0052$<br>
The margin of error is: $ME_{China}=`r 1.96*0.0223`$, $ME_{India}=`r 1.96*0.0052`$
</div> 
...
<div class="alert alert-success">
ME increases when p increases, until it reaches a maximum at 0.5. <br>
Then ME decreases as p increases to 1. 
</div> 
...
```{r}
summary(p_hats)
m<-round(mean(p_hats),4)
sd<-round(sd(p_hats),4)
```

<div class="alert alert-success">
The sampling distribution is:<br>
- unimodal and symmetric<br>
- centered at a mean of about `r m`<br>
- with a standard deviation of `r sd`
</div> 
...
par(mfrow = c(2, 2))
hist(p_hats, main = "p = 0.1, n = 1040", xlim = c(0, 0.18))
hist(sim(400,0.1), main = "p = 0.1, n = 400", xlim = c(0, 0.18))
hist(sim(1040,0.02), main = "p = 0.02, n = 1040", xlim = c(0, 0.18))
hist(sim(400,0.02), main = "p = 0.02, n = 400", xlim = c(0, 0.18))
```
...
inference(sp2005$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")   
inference(sp2012$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")    
```
...
<div class="alert alert-success">
The worst case $p$ would for a margin of error would be 0.5, so we will use this as a basis to compute the required n value. At 95% confidence level and with z=1.96, we would need to survey `r n` people.
</div> 
